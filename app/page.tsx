...
export default function Home() {
  ...
  const [activeChapterName, setActiveChapterName] = useState<keyof typeof chapters>('bored');
      ...
      <div ref={mainRef} className="z-10 items-center justify-between font-mono text-3xl flex flex-col snap-y snap-mandatory overflow-y-scroll h-screen w-full scrollbar-hide">
        ...
        <div className="min-h-screen snap-always snap-center flex justify-start w-full items-center " id="apes">
          <div className='w-half md:w-1/3 border-2 border-[#EC4B28] bg-black bg-opacity-40 font-bold text-[#EC4B28] rounded-lg p-8 text-center flex flex-col gap-4'>
            <div>BY APES, FOR APES
            </div>
            <div className='text-sm'>Network and meet-up with apes close to you</div>
        ...
        <div className="min-h-screen snap-always snap-center flex items-center justify-end w-full " id="mbas">
          <div className='w-half  md:w-1/3  border-2 border-[#EC4B28] bg-black bg-opacity-40 font-bold text-[#EC4B28] rounded-lg p-8 text-center flex flex-col gap-4'>
            <div>LOCAL CLUBS AND MBAS
            </div>
            <div className='text-sm'>Discover local businesses and clubs by fellow apes</div>
        ...
        <div className="min-h-screen snap-always snap-center flex items-center justify-start w-full" id="events">
          <div className='w-half md:w-1/3  border-2 border-[#EC4B28] bg-black bg-opacity-40 font-bold text-[#EC4B28] rounded-lg p-8 text-center flex flex-col gap-4'>
            <div>EVENTS AND TRAVEL
            </div>
            <div className='text-sm'>Plan your trip and join in exclusive events</div>
            <div className='flex justify-center'>
        ...
        <div className="min-h-screen snap-always snap-center flex items-center justify-center w-full" id="register">
          <div className='w-half border-2 border-[#EC4B28] bg-black bg-opacity-40 font-bold text-[#EC4B28] rounded-lg p-8 text-center'>
            <div>GET NOTIFIED ON LAUNCH
            </div>
            <div >
              <form >
                <div className='flex flex-col md:flex-row gap-2 pt-4 '>
                  <input className='w-full rounded-lg p-2 border-[#EC4B28] border-2 bg-transparent' type="email" required placeholder="Email" value="" />
                  <button type="submit" >
                    Join
                  </button>
                </div>
        ...
        {/* 
        <div className="min-h-screen snap-always snap-center flex items-center justify-center" id="register">

          <div className='w-half border-2 border-[#EC4B28] bg-black bg-opacity-40 font-bold text-[#EC4B28] rounded-lg p-8 text-center '>
            <div className='max-w-full text-wrap'>GET NOTIFIED ON LAUNCH
            </div>
            <form >
              <div className='max-w-full text-wrap flex flex-row gap-2 pt-4 '>
                <input className='rounded-lg p-2 border-[#EC4B28] border-2 bg-transparent' type="email" required placeholder="Email" value="" />
                <button type="submit" >
                  Join
                </button>
              </div>
